...
func testAccCheckAWSAPIGatewayResourceExists(n string, res *apigateway.Resource) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		req := &apigateway.GetResourceInput{
			...
			RestApiId:  aws.String(s.RootModule().Resources["aws_api_gateway.test"].Primary.ID),
		}
...
resource "aws_api_gateway_resource" "test" {
  api_id = "${aws_api_gateway.test.id}"
  parent_resource_id = "${aws_api_gateway.test.root_resource_id}"
  path_part = "test"
}
`
